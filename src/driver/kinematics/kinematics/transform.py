...
# Link length(m) （连杆长度(m))
# The height of the base; here, the origin of the first coordinate system has been aligned with the origin of the second coordinate system （底座的高度，这里把第一个坐标系和第二个坐标的原点重合到一起了)
machine_type = os.environ.get('MACHINE_TYPE')
if machine_type == 'MentorPi_Acker_Pro':
    base_link = 0.05 + 0.0654868 + 0.0338648 + 0.0772047
elif machine_type == 'MentorPi_Mecanum_Pro':
    base_link = 0.22736 
elif machine_type == 'MentorPi_Tank_Pro':
    base_link = 0.127 + 0.0338648 + 0.0772047 

link1 = 0.130
link2 = 0.130

# When calculating tool_link, use the value link3 + tool_link because the origin of the end-effector's coordinate system has been aligned with the previous one （计算tool_link时取值为link3 + tool_link，因为把末端的坐标系原点和前一个重合到一起了）
# Here, tool_link refers to the actual length of the gripper （这里的tool_link指实际上的夹持器长度）
link3 = 0.55
tool_link = 0.117

# Joint angle limits depend on collision avoidance and the rotation range of the servo （各关节角度限制，取决于是否碰撞以及舵机的转动范围）
# Add an extra 0.2 to prevent numerical instability during calculations, making it slightly larger than the set value （多加0.2为了防止计算时数值的不稳定，会比设定值大一点点）
joint1 = [-120.2, 120.2]
joint2 = [-180.2, 0.2]
joint3 = [-120.2, 120.2]
joint4 = [-200.2, 20.2]
...
#         Servomotor pulse width range, midpoint value, corresponding angle range, midpoint value （舵机脉宽范围，中位值，对应的角度范围，中位值)
...
# Determine if it is a rotation matrix (判断是否为旋转矩阵)
