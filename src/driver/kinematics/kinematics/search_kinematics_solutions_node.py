...
# @data:2023/03/20
# @author:aiden
# Real-time angle feedback based on the current position and (实时获取角度反馈，根据当前位置和）
# Obtain the optimal solution based on the minimum deviation from the target position (目标位置的最小差值来获取最优解）
import rclpy
import numpy as np
...
fk = ForwardKinematics(debug=False)  # Disable printing (不开启打印）
class SearchKinematicsSolutionsNode(Node):
    ...
    def set_link_srv(self, request, response):
        # Set the length of the link (设置link长度)
    ...
    def set_joint_value_target_srv(self, request, response):
        # Forward kinematics solution （正运动学解)
    ...
    def set_pose_target(self, position, pitch, pitch_range, resolution):
        # Inverse kinematics solution: obtain the optimal solution (minimizing the rotation of all motors) （逆运动学解，获取最优解(所有电机转动最小))
