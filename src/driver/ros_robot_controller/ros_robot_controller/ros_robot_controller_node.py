...
import signal
import threading
import yaml  # Imported PyYAML （已导入 PyYAML)
...
class RosRobotController(Node):
    ...
    def __init__(self, name):
        ...
        self.create_subscription(Bool, '~/enable_reception', self._on_enable_reception, 1)
        ...
        # Load and set servo offset from the YAML file (加载并设置舵机偏移量从 YAML 文件)
    ...
    def load_servo_offsets(self):
        ...
        """
        config_path = os.path.expanduser('~/.ros/servo_config.yaml')
        try:
            with open(config_path, 'r') as file:
                config = yaml.safe_load(file)

            # Ensure the config is a dictionary (确保config是字典）
            if not isinstance(config, dict):
                self.get_logger().error(f"Invalid YAML configuration file format: {config_path}. A dictionary format is required.")
                return

            # Iterate from ID 1 to ID 4 and set the offset (遍历ID1到ID4并设置偏移量)
            for servo_id in range(1, 5):
                ...
                try:
                    ...
                    self.get_logger().info(f"The offset for servo {servo_id} has been set to {offset}")
                except Exception as e:
                    self.get_logger().error(f"Error setting offset for servo {servo_id}: {e}")

        except FileNotFoundError:
            self.get_logger().info(f"No custom servo config file at {config_path}, using default offsets.")
        except yaml.YAMLError as e:
            self.get_logger().error(f"Error occurred while parsing the YAML file: {e} (解析 YAML 文件时出错: {e})")
        except Exception as e:
            self.get_logger().error(f"Error occurred while reading the configuration file: {e} (读取配置文件时出错: {e})")
    ...
    def _on_enable_reception(self, msg):
