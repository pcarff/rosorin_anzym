...
class ControllerManager(Node):
    def __init__(self, name):
        ...
        # Read configuration parameters (读取配置参数）
        self.base_frame = self.get_parameter('base_frame').value
        
        # Initialization of the trajectory_controller (trajectory_controller的初始化）
        self.controllers = {}
        connected_ids = {}
        ...
        self.servo_manager.connect()  # Check if the specified servo is connected (检查是否有给定的舵机已连接)
        ...
        # Ensure that ros_robot_controller has completed initialization (确保ros_robot_controller已完成初始化)
    ...
    def publish_joint_states(self):
        while True:
            ...
            for i in positions:
                ...
                if int(i) == 10:
                    positions[i].position = 1000 - positions[i].position
...
def main():
    ...
    rclpy.spin(node)  # Loop to wait for ROS2 to exit (循环等待ROS2退出)
