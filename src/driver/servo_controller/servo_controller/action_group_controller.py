...
class ActionGroupController:
    ...
    def run_action(self, action_name):
        '''
        Running the action group, unable to send the stop signal （运行动作组，无法发送stop停止信号)
        :param action_name: Action group name, of string type （动作组名字 ， 字符串类型)
        :param times:  Number of runs （运行次数)
        :return:
        '''
        ...
        if os.path.exists(action_name) is True:
            if self.running_action is False:
                ...
                while True:
                    ...
                    else:   # Exit only after completion （运行完才退出)
                        break
            ...
            self.running_action = False
            print("Action group file not found: （未能找到动作组文件:）", action_name)
