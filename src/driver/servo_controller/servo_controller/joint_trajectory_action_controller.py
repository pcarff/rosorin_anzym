...
class JointTrajectoryActionController(Node):
    ...
    def follow_trajectory_callback(self, goal_handle):
        ...
        lookup = []
        for joint in self.joint_names:
            lookup.append(traj.joint_names.index(joint))  # Convert the order of joints to numerical indices （将joint的顺序转为数字索引)
        ...
        for i in range(1, num_points):
            # Subtracting the timestamp of the previous point from the timestamp of the next point gives the time required for the trajectory to run (下一个减去上一个的时间戳算出来就是这个轨迹运行需要的时间)
        ...
        for i in range(num_points):  # Traverse all trajectory points and store them back into the list （遍历所有轨迹点，将他重新存储到列表里)
